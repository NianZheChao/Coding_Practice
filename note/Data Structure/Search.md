...
- 删除结点p为叶子
  ...
  p无子树，只需将p的parent中指向p的指针域置空

- p有一个孩子

  将lchild（或rchild）与p的双亲连接，即可删去p

- p有两个孩子

  - 方案一：删除p的中序遍历的直接前驱结点，使用该结点值替换p，即查找p的左子树上最右下方的结点
  - 方案二：删除p的中序遍历的直接后继，使用该结点值替换p，即查找p的右子树上最左下方的结点
...
- 就维护表的有序性而言，BST无需移动结点，只需修改指针即可完成插入和删除操作，且平均的执行时间均为$O(log_2n)$，因此更有效

#### 平衡二叉树（Adelson-Velsky-Landis Tree,AVL）

**定义**：平衡二叉树或是一棵空树，或是具有下列性质的BST：它的左子树和右子树都是平衡二叉树；左子树和右子树高度差的绝对值（平衡因子）不超过1。

#### 非AVL树的调整

在BST上插入或删除结点后，可能使树失去平衡，需对失去平衡的树进行平衡化调整。

- LL型处理

  在一个只有左孩子B的结点C上插入了一个左孩子A，使C的平衡因子增大成为不平衡的二叉排序树；平衡处理为：将根节点C顺时针旋转，成为其左子树B的右子树，而原来B的右子树A变为C的左子树。

- LR型处理

  结点C只有左孩子A，在A上插入了一个右孩子B。平衡处理：将B变到A与C之间，使之成为LL型，然后按LL型处理。

![1590053767471](../images/1590053767471.png)
...
![1590053897823](../images/1590053897823.png)
